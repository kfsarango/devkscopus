...
class consultas(generics.ListAPIView):
	...
	def get_queryset(self):
		...
		if myProyect == None:
			...
			myProyect.register = datetime.datetime.now()
		...
		for x in range(anio_start,YEAR):
			...
			while anio_range_limit <= YEAR:
				query_sentence = sentence + ' AND PUBYEAR AFT ' + str(anio_start) + ' AND PUBYEAR BEF '+ str(anio_range_limit)
				rtaSubconsulta = resultados_subconsulta(query_sentence)
				publicaciones = rtaSubconsulta[0]
				urlSunconsulta = rtaSubconsulta[1]
		...
		for sc in listSentences:
			...
			subconsultaObj.register = datetime.datetime.now()
	...
	def get_queryset(self):
		...
		for c in consultas:
			...
			for s_c in sub_consultas:
				parametros = '?query='+s_c.nombre.replace(' ','%20')+'&apikey='+AK+'&view=COMPLETE&httpAccept=application/json'
				url = HOST+parametros
				pag = 24
				count = '&count=1'
				...
				while myCont < countResults:
					...
					jsonObj.register = datetime.datetime.now()
			...
			for pub in entries:
				...
				if affs != None:
					...
					for a in affs:
						...
						pub_affObj = ScpPublicationAffiliation.objects.filter(scp_publication=publicacion.id, scp_affiliation=afiliacion.id).first()
						if pub_affObj == None:
					...
					for a in auths:
						...
						if affids != None:
							for af in affids:
								...
								au_affObj = ScpAuthorAffiliation.objects.filter(scp_publication=publicacion.id, scp_author= autor.id).first()
								if au_affObj == None:
...
def guardar_consulta(sentence,idProyecto):
	...
	consultaObj.register = datetime.datetime.now()
